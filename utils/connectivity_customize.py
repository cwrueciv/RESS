...
def local_node_connectivity(G, source, target, cutoff=None):
    ...
    for i in range(min(possible, cutoff)):
        try:
            ...
            individual_pahts.append(path)
        except nx.NetworkXNoPath:
            break

    return individual_pahts
...
def all_pairs_node_connectivity(G, nbunch=None, cutoff=None):
    ...
    for u, v in iter_func(nbunch, 2):
        individual_paths = local_node_connectivity(G, u, v, cutoff=cutoff)

        if individual_paths:
            pathes_reliability = []
            pathes_length = []
            for singal_path in individual_paths:
                single_edges = [singal_path[i:i+2] for i in range(len(singal_path) - 1)]
                reliablity = np.prod([G[i[0]][i[1]]['reliability'] for i in single_edges])
                path_distance = np.sum([G[i[0]][i[1]]['weight'] for i in single_edges])
                pathes_reliability.append(reliablity)
                pathes_length.append(path_distance)

            L_max = max(pathes_length)
            Sum_length = np.sum([L_max / i for i in pathes_length] )
            pathes_weight = [L_max / (i * Sum_length) for i in pathes_length]

            node_weight = sum([x*y for x, y in zip(pathes_weight, pathes_reliability)])
        else:
            node_weight = 0

        all_pairs[u][v] = node_weight
        if not directed:
            all_pairs[v][u] = node_weight
