...
G_origin, _, _ = initial_situation()
...
class QNet_Agent(object):
    ...
    def store_model(self):
        torch.save(self.nn.state_dict(), '../results/RL learning global model.pt')
    ...
    def select_action(self, state, epsilon, inspected_list):
        ...
        if random_for_egreedy > epsilon:
            ...
            with torch.no_grad():
                ...
                action_from_nn[0][inspected_list] = -np.inf
...
for i_episode in range(num_episodes):
    G_nump, resil, node_resil_list = initial_situation()
        ...
        if done or (step > 200):
            ...
            if (i_episode % report_interval == 0):
                print("\n*** Episode %i *** \
                      ...
                      %
                      (i_episode,
                       np.sum(resilience_curve) / len(resilience_curve),
                       epsilon,
                       step
                       )
                      )
...
save_file('../results/resilience curve with global Graph learning.pkl', final)
save_file('../results/repaire sequence with global Graph learning.pkl', final_inspected_list)
